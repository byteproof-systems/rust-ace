...
#[export_name = "_ZN15ACE_TSS_AdapterC1EPvPFvS0_E"]
pub unsafe extern "C-unwind" fn __xtu__ZN15ACE_TSS_AdapterC1EPvPFvS0_E(
    __this: *mut ACE_TSS_Adapter,
    __a0: *mut libc::c_void,
    __a1: Option<unsafe extern "C-unwind" fn(*mut libc::c_void)>,
) {
    ACE_TSS_Adapter::new_at(__this, __a0, __a1)
}
#[export_name = "_ZN15ACE_TSS_Adapter7cleanupEv"]
pub unsafe extern "C-unwind" fn __xtu__ZN15ACE_TSS_Adapter7cleanupEv(
    __this: *mut ACE_TSS_Adapter,
) {
    unsafe { ACE_TSS_Adapter::cleanup(__this) }
}
#[no_mangle]
pub unsafe extern "C-unwind" fn ACE_TSS_C_cleanup(mut object: *mut libc::c_void) {
    unsafe {
        {
            if ((((!(object).is_null()) as libc::c_int) as libc::c_int) != 0) {
                let mut tss_adapter: *mut ACE_TSS_Adapter = (((object
                    as *mut ACE_TSS_Adapter)) as *mut ACE_TSS_Adapter);
                <ACE_TSS_Adapter>::cleanup((tss_adapter) as *mut ACE_TSS_Adapter);
                {
                    let __d = tss_adapter;
                    if !__d.is_null() {
                        let _ = Box::from_raw(__d as *mut ACE_TSS_Adapter);
                    }
                };
            }
        }
